...
public class Device {
    ...
    public static Device getDevice(String deviceId, String accessToken) throws Exception {
        ...
        conn.connect();
        return new Device(new JSONObject(new JSONTokener(conn.getInputStream().toString())));
    }
    ...
    public static ArrayList<Device> getDevices(String accessToken) throws Exception {
        ...
        conn.setRequestProperty("Authorization", accessToken);
        conn.connect();
        JSONArray ja = new JSONArray(new JSONTokener(conn.getInputStream().toString()));
    ...
    @RequiresApi(api = Build.VERSION_CODES.N)
    private void updateFields(JSONObject jo) throws JSONException {
        try {
    ...
    public Integer callFunction(String functionName, String arg, String accessToken) throws Exception {
        ...
        if (resp == 200) {
            JSONObject jo = new JSONObject(new JSONTokener(conn.getInputStream().toString()));
